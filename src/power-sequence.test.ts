...
describe('open neon power-up sequence',()=>{
  it('keeps the mounted sign dark through lane power-up, including Lane 8',()=>{expect(neonPresentationAt(0)).toBe('DARK');expect(neonPresentationAt(LANE_EIGHT_COMPLETE_MS)).toBe('DARK');expect(neonPresentationAt(LANE_POWER_COMPLETE_MS-1)).toBe('DARK')})
  it('keeps the sign dark after the complete lane sequence',()=>{expect(OPEN_SIGN_DARK_MS).toBe(LANE_POWER_COMPLETE_MS);expect(neonPresentationAt(OPEN_SIGN_DARK_MS)).toBe('DARK')})
  ...
  it('never removes the physical sign while it is unlit',()=>{expect(neonPresentationAt(0)).toBe('DARK');expect(neonPresentationAt(OPEN_SIGN_DARK_MS)).toBe('DARK')})
